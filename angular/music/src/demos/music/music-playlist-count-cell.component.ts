...
@Component({
  ...
  template: `
    <div style="display:flex;flex-direction:column;gap:4px;align-items:flex-start;">
      <div style="font-size:14px;" [style.color]="c.gray">{{ row.playlistCount.toLocaleString() }}</div>
...
export class MusicPlaylistCountCellComponent {
  ...
  get tagText(): string {
    return `↑ +${this.row.playlistCountGrowth} (${musicNumber(this.row, "playlistCountGrowthPercent").toFixed(2)}%)`;
  }
}
