  ...
  if (h.accessor === "priority") {
    return {
      ...
      cellRenderer: ({ row }: { row: Record<string, unknown> }) => {
        ...
        return `<span style="color:${color};font-weight:bold;cursor:pointer" title="Click to filter">${p}</span>`;
      },
    };
  }
  if (h.accessor === "status") {
    return {
      ...
      cellRenderer: ({ row }: { row: Record<string, unknown> }) => {
        ...
        return `<span style="background:${bg};color:${color};padding:4px 8px;border-radius:4px;font-size:12px;font-weight:bold;cursor:pointer" title="Click to change">${s}</span>`;
      },
    };
  }
