<script lang="ts">
  ...
  const pctStyle = $derived.by(() => {
    ...
    if (row.profitMargin >= 0.7) {
      color = colors.success.high.color;
      fontWeight = colors.success.high.fontWeight;
    } else if (row.profitMargin >= 0.5) color = colors.success.medium;
    else if (row.profitMargin >= 0.4) color = colors.success.low;
    else if (row.profitMargin >= 0.3) color = colors.info;
    else color = colors.warning;
    const barColor =
      row.profitMargin >= 0.5
        ? colors.progressColors.high
        : row.profitMargin >= 0.3
          ? colors.progressColors.medium
          : colors.progressColors.low;
    return { color, fontWeight, barColor, pct: `${(row.profitMargin * 100).toFixed(1)}%`, w: row.profitMargin * 100 };
  });
</script>
