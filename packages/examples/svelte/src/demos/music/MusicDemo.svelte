<script lang="ts">
  ...
  function tag(text: string, color: "green" | "red" | "default", themeColors: Record<string, string>): HTMLElement {
    ...
    const span = el("span", {
      ...
      padding: "0 7px", fontSize: "11px", lineHeight: "20px",
  ...
  function buildMusicHeaders(t?: string): HeaderObject[] {
    const c = getMusicThemeColors(t);
  ...
  const headers = $derived(buildMusicHeaders(theme));
</script>
