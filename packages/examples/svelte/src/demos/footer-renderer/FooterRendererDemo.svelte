<script lang="ts">
  ...
  function footerRenderer(fp: FooterRendererProps): HTMLElement {
    ...
    Object.assign(wrapper.style, {
      ...
      padding: "16px 20px", backgroundColor: c.background,
    ...
    function makeBtn(label: string, onClick: () => void, disabled: boolean, active = false) {
      ...
      Object.assign(btn.style, {
        ...
        cursor: disabled ? "not-allowed" : "pointer", transition: "all 0.2s", minWidth: "40px",
      });
    ...
    for (let p = 1; p <= fp.totalPages; p++) {
      const btn = makeBtn(String(p), () => fp.onPageChange(p), false, p === fp.currentPage);
