...
const CATEGORIES = [
  "Smart Contract", "DeFi", "Meme", "Layer 2", "Gaming", "AI & Compute", "Privacy", "Payments", "Storage",
];
...
function randomSymbol(rand: () => number): string {
  ...
  let symbol = "";
  for (let i = 0; i < len; i++) symbol += SYMBOL_CHARS[Math.floor(rand() * SYMBOL_CHARS.length)];
  return symbol;
}
...
export function generateCryptoData(count = 200): CryptoCoin[] {
  ...
  coins.forEach((coin, idx) => { coin.rank = idx + 1; });
  return coins;
}
...
export const cryptoHeaders: SvelteColumnDef<CryptoCoin>[] = [
...
export const cryptoData = generateCryptoData(200);
