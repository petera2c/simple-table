...
function shallowTablePropsChanged<TData extends ReactDefaultRowData>(
  prev: SimpleTableReactProps<TData>,
  next: SimpleTableReactProps<TData>,
): boolean {
  const keys = new Set([...Object.keys(prev as object), ...Object.keys(next as object)]) as Set<
    keyof SimpleTableReactProps<TData>
  >;
 ...
 * />
 */
function SimpleTableInner<TData extends ReactDefaultRowData = ReactDefaultRowData>(
  props: SimpleTableReactProps<TData>,
  ref: React.ForwardedRef<TableAPI<TData>>,
) {
   ...
   */
  const syncedDefaultHeadersRef = useRef<ReadonlyArray<ReactColumnDef<TData, any>> | undefined>(
    undefined,
  );
  /** Last `rows` array whose content was applied via `update`. */
  const syncedRowsRef = useRef<SimpleTableReactProps<TData>["rows"] | undefined>(undefined);
  const lastSyncedPropsRef = useRef<SimpleTableReactProps<TData> | null>(null);
  ...
  useLayoutEffect(() => {
    ...
    if (ref) {
      ...
      const api = instance.getAPI() as unknown as TableAPI<TData>;
  ...
  useLayoutEffect(() => {
    ...
    const rowsUnchanged = rowsShallowUnchanged(
      ...
      reactProps.getRowId as Parameters<typeof rowsShallowUnchanged>[2],
    );
    syncedRowsRef.current = reactProps.rows;
    if (rowsUnchanged) {
      delete patch.rows;
...
const SimpleTable = React.forwardRef(SimpleTableInner) as SimpleTableComponent & {
  displayName?: string;
};
