<script lang="ts">
  ...
  const isDark = $derived(theme === "custom-dark" || theme === "dark" || theme === "modern-dark");
  const footerColors = $derived(isDark ? CRM_FOOTER_COLORS_DARK : CRM_FOOTER_COLORS_LIGHT);
  ...
  function getCRMHeaders(dark: boolean): HeaderObject[] {
    const colors = dark ? CRM_THEME_COLORS_DARK : CRM_THEME_COLORS_LIGHT;
    ...
    const contactRenderer: CellRenderer = ({ row }) => {
      ...
      companyEl.appendChild(el("span", { fontSize: "12px", color: colors.textTertiary }, "@"));
    ...
    const signalRenderer: CellRenderer = ({ row }) => {
      ...
      line2.appendChild(el("span", { fontWeight: "600" }, "Keyword:"));
  ...
  function createCRMFooter(props: FooterRendererProps, fc: typeof CRM_FOOTER_COLORS_LIGHT, rpp: number): HTMLElement {
    const c = fc;
    const wrapper = el("div", {
    ...
    for (const opt of [25, 50, 100, 200, 10000]) {
      ...
      if (opt === rpp) option.selected = true;
  ...
  const headers = $derived(getCRMHeaders(isDark));
</script>
...
<div class={`custom-theme-container theme-${isDark ? "custom-dark" : "custom-light"}`}>
  <SimpleTable
    ...
    shouldPaginate={true}
    theme="custom"
    footerRenderer={(props) => createCRMFooter(props, footerColors, rowsPerPage)}
  />
</div>
