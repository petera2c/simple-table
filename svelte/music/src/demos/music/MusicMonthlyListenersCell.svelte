<script lang="ts">
  ...
  const isPos = $derived(row.monthlyListenersChange >= 0);
  const pct = $derived(Math.abs(row.monthlyListenersChangePercent).toFixed(2));
  const tagText = $derived(
    `${isPos ? "↑" : "↓"} ${isPos ? "+" : ""}${row.monthlyListenersChangeFormatted} (${pct}%)`,
  );
</script>

<div style="display:flex;flex-direction:column;gap:4px;">
  <div style="font-size:14px;color:{c.gray};">{row.monthlyListenersFormatted}</div>
