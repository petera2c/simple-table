<script lang="ts">
  ...
  const avatarStyle = $derived.by(() => {
    let hash = 0;
    for (let i = 0; i < row.artistName.length; i++) {
      hash = row.artistName.charCodeAt(i) + ((hash << 5) - hash);
    }
...
<div style="display:flex;align-items:center;gap:12px;">
  <div style={avatarStyle}>{row.artistName.charAt(0).toUpperCase()}</div>
  <div style="display:flex;flex-direction:column;gap:6px;flex:1;">
    <span style="font-weight:500;font-size:14px;color:{c.gray};">{row.artistName}</span>
    <div style="display:flex;gap:6px;flex-wrap:wrap;">
      <MusicTag text={row.growthStatus} variant="default" c={c} />
      <MusicTag text={row.mood} variant="default" c={c} />
      <MusicTag text={row.genre} variant="default" c={c} />
    </div>
  </div>
</div>
