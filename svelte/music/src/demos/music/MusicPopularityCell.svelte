<script lang="ts">
  ...
  const isPos = $derived(row.popularityChangePercent >= 0);
  const arrow = $derived(isPos ? "↑" : "↓");
  const tagText = $derived(`${arrow} ${Math.abs(row.popularityChangePercent).toFixed(2)}%`);
</script>

<div style="display:flex;justify-content:center;">
  <div style="display:flex;flex-direction:column;gap:4px;align-items:flex-end;">
    <span style="font-size:14px;color:{c.gray};">{row.popularity}/100</span>
