...
import type { Theme, VueColumnDef, CellClickProps, CellRendererProps, GetRowIdParams } from "@simple-table/vue";
...
const headers: VueColumnDef<ProjectTask>[] = cellClickingHeaders.map((col) => {
...
function handleCellClick({ accessor, rowIndex, value, row }: CellClickProps<ProjectTask>) {
  switch (accessor) {
    ...
    case "status": {
      ...
      rows.value = rows.value.map((t) => (t.id === row.id ? { ...t, status: next } : t));
      clickInfo.value = `Status: "${value}" → "${next}"`;
      break;
    }
    case "details":
      selectedTask.value = row;
      clickInfo.value = `Opening details for: ${row.task}`;
      break;
    case "estimatedHours": {
      const n = Math.min(row.estimatedHours + 2, 40);
      rows.value = rows.value.map((t) => (t.id === row.id ? { ...t, estimatedHours: n } : t));
      clickInfo.value = `Est. hours: ${row.estimatedHours}h → ${n}h`;
      break;
    }
    case "completedHours": {
      const n = Math.min(row.completedHours + 1, row.estimatedHours);
      rows.value = rows.value.map((t) => (t.id === row.id ? { ...t, completedHours: n } : t));
      clickInfo.value = `Done hours: ${row.completedHours}h → ${n}h`;
      break;
    }
    default:
