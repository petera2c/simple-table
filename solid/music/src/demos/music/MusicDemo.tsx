  ...
  return [
    ...
    },
    {
      ...
      children: [
        ...
        { accessor: "monthlyListeners28DayGrowth", label: "28-Day Growth", width: "auto", sortable: true, editable: false, align: "right", type: "number", showWhen: "parentExpanded", cellRenderer: ({ row, theme }: CellRendererProps<MusicArtist>) => { const d = row; const growth = musicNumber(d, "monthlyListeners28DayGrowth"); return <GrowthMetric value={growth} growthPercent={musicNumber(d, "monthlyListeners28DayGrowthPercent")} isPositive={growth >= 0} theme={theme} align="right" />; } },
        { accessor: "monthlyListeners60DayGrowth", label: "60-Day Growth", width: "auto", sortable: true, editable: false, align: "right", type: "number", showWhen: "parentExpanded", cellRenderer: ({ row, theme }: CellRendererProps<MusicArtist>) => { const d = row; const growth = musicNumber(d, "monthlyListeners60DayGrowth"); return <GrowthMetric value={growth} growthPercent={musicNumber(d, "monthlyListeners60DayGrowthPercent")} isPositive={growth >= 0} theme={theme} align="right" />; } },
      ],
    },
    { accessor: "conversionRate", label: "Conversion Rate", width: "auto", sortable: true, editable: false, align: "right", type: "number", cellRenderer: ({ row, theme }: CellRendererProps<MusicArtist>) => { const d = row; const c = getMusicThemeColors(theme); return <span style={{ color: c.gray }}>{musicNumber(d, "conversionRate").toFixed(2)}%</span>; } },
    { accessor: "reachFollowersRatio", label: "Reach/Followers Ratio", width: "auto", sortable: true, editable: false, align: "right", type: "number", cellRenderer: ({ row, theme }: CellRendererProps<MusicArtist>) => { const d = row; const c = getMusicThemeColors(theme); return <span style={{ color: c.gray }}>{musicNumber(d, "reachFollowersRatio").toFixed(1)}x</span>; } },
  ];
}
...
export default function MusicDemo(props: { height?: string | number; theme?: Theme }) {
  ...
  return (
    <div class="music-theme-container" style={{ "font-family": "Inter" }}>
      <SimpleTable autoExpandColumns columnReordering columnResizing customTheme={{ headerHeight: 30, rowHeight: 85 }} columns={getMusicHeaders()} getRowId={({ row }) => row.id} height={props.height ?? "400px"} ref={(api) => (tableRef = api)} rows={musicData} selectableCells theme={props.theme} />
    </div>
  );
}
