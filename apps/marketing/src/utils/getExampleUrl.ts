 ...
 * @param currentTheme - The current website theme ("light" or "dark")
 * @returns The example URL with the appropriate table theme parameter
 */
export function getExampleUrl(examplePath: string, currentTheme: "light" | "dark"): string {
  ...
  // All other examples default to modern-* themes (matches ExamplesProvider)
  return `${examplePath}?theme=${mapWebsiteThemeToTableTheme(currentTheme)}`;
}

/**
 * Generates the URL for the default example page with the correct theme parameter
 * @param currentTheme - The current website theme ("light" or "dark")
 * @returns The default example URL with the appropriate table theme parameter
 */
